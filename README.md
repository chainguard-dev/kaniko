...
- [Using kaniko](#using-kaniko)
  ...
  - [Using S3 Presigned URLs](#using-s3-presigned-urls)
...
This flag can be used together with `--tar-path` to both save locally and upload remotely.

#### Flag `--upload-tar-retry`

Set this flag as `--upload-tar-retry=<count>` to specify the number of retry attempts for the upload operation when using `--upload-tar`. The default is `0` (no retries).

Retries use exponential backoff starting at 1 second (1s, 2s, 4s, 8s, etc.). Both network errors and HTTP server errors will trigger retries.
